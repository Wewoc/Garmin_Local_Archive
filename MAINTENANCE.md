...
**Token persistence** — after the first successful SSO login, `garmin_api.login()` calls `garmin_security.save_token()` to encrypt the Garmin OAuth token with AES-256-GCM and store it at `LOG_DIR/garmin_token.enc`. The garminconnect library (≥ 0.2.40) writes the token as `garmin_tokens.json` into `GARMIN_TOKEN_DIR` — `save_token()` reads this file, encrypts its contents, writes `garmin_token.enc`, then removes `GARMIN_TOKEN_DIR`. On subsequent runs, `load_token()` decrypts the file and writes `garmin_tokens.json` back so the library can read it directly — the file is removed immediately after login. Plaintext never persists on disk outside the library login window. The encryption key is user-defined (set once via popup on first setup) and stored in Windows Credential Manager under `GarminLocalArchive / token_enc_key`. If the token is rejected by Garmin, `clear_token()` removes it and a warning popup asks the user before triggering a new SSO login. If the WCM key is lost (e.g. after a Windows update), the user re-enters it via popup. MFA is supported via a dedicated input popup — users who have MFA enabled on their Garmin account will be prompted for the code automatically.
