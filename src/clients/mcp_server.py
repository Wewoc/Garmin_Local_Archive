...
Standalone MCP server process. Registers six MCP tools via the official
mcp SDK (mcp>=1.28,<2, verified against mcp==1.29.0): query_fit_activities,
query_raw, get_archive_metadata, list_available_fields are defined directly
here as thin 1:1 @mcp.tool() wrappers around maps/mcp_map.py, with no
broker/delegation logic of their own. query_health and query_context
(Codereview v1.7.0.1, Baustein 2.2) live in their own files instead —
clients/mcp_health.py / clients/mcp_context.py — imported here and
registered programmatically (`query_health = mcp.tool()(query_health)`)
rather than with `@mcp.tool()` at the definition site, since the shared
`mcp` FastMCP instance lives in this module and importing it into those
two files would be circular; see clients/mcp_query_common.py's module
docstring for the full reasoning. Their own routing/alias/fuzzy-match
logic lives in those two files (and clients/mcp_field_registry.py for
the underlying data), not in mcp_map.py — mcp_map.py's own
query_health()/query_context() stay the thin brokers they always were,
reachable only via the (currently always-false) live-archive branch of
_route_query().
...
# mcp_field_registry's five names (FIELD_UNITS, HEALTH_FIELD_ALIASES,
# HEALTH_FIELD_AMBIGUOUS, CONTEXT_FIELD_ALIASES, CONTEXT_FIELD_AMBIGUOUS)
# are no longer imported here directly (Codereview v1.7.0.1, Baustein
# 2.2) — the functions that used them (query_health, query_context,
# _enrich_with_units, _get_field_unit) all moved out to
# clients/mcp_health.py / mcp_context.py / mcp_query_common.py, which
# import mcp_field_registry themselves.
...
# _CONTEXT_CATEGORY_BUNDLES moved to mcp_field_registry.py (Codereview
# v1.7.0.1, Baustein 2.2) — needed by both mcp_health.py and
# mcp_context.py, living in the neutral shared registry avoids a direct
# file dependency between the two.
...
# ══════════════════════════════════════════════════════════════════════════════
#  Query-serving layer — split across four files as of Codereview
#  v1.7.0.1 (Baustein 2.1 + 2.2):
#    clients/mcp_field_registry.py — FIELD_UNITS, HEALTH_FIELD_ALIASES,
#      HEALTH_FIELD_AMBIGUOUS, CONTEXT_FIELD_ALIASES,
#      CONTEXT_FIELD_AMBIGUOUS, _CONTEXT_CATEGORY_BUNDLES (pure data)
#    clients/mcp_query_common.py — _route_query, _get_field_unit,
#      _enrich_with_units (shared helpers, no mcp/mcp_sql/mcp_map dep)
#    clients/mcp_health.py — query_health()
#    clients/mcp_context.py — query_context(), _resolve_context_bundle(),
#      _fetch_context_field()
#  See mcp_query_common.py's module docstring for why the split needed a
#  separate helpers module (circular-import avoidance) and why
#  query_health()/query_context() are imported + registered
#  programmatically below instead of staying here with `@mcp.tool()`.
# ══════════════════════════════════════════════════════════════════════════════

from mcp_query_common import _route_query, _get_field_unit  # noqa: E402
...
# query_health moved to clients/mcp_health.py (Codereview v1.7.0.1,
# Baustein 2.2). Same programmatic-registration reasoning as
# query_context below — see that comment for the full rationale.
from mcp_health import query_health  # noqa: E402 — after mcp = FastMCP(...) construction, v1.7.0.1
query_health = mcp.tool()(query_health)





# query_context moved to clients/mcp_context.py (Codereview v1.7.0.1,
# Baustein 2.2). Imported as a plain function and registered here
# programmatically (f = dec(f), same as `@mcp.tool()` at the definition
# site would do) — avoids mcp_context.py needing to import `mcp` back
# from this module, which would be circular. mcp_server.query_context
# still resolves exactly as before for tests/test_mcp.py's qualified
# `mcp_server.query_context(...)` calls.
from mcp_context import query_context  # noqa: E402 — after mcp = FastMCP(...) construction, v1.7.0.1
query_context = mcp.tool()(query_context)
...
def list_available_fields(domain: str | None = None) -> dict:
    ...
    existing callers reading "fields" (e.g. mcp_context.py's
    _resolve_context_bundle(), which iterates the plain name lists) are
    unaffected. See FIELD_UNITS in mcp_field_registry.py for the unit
