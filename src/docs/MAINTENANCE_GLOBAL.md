...
| `updater.py` (v1.7.2.4, v1.7.2.4.1) | Self-updater core (T3 first, T2 added v1.7.2.4-Nacherweiterung): `resolve_release_asset()` (GitHub release asset lookup by exact name), `prepare_update()` (download + SHA256-verify + extract to a sibling `_update_pending/` folder; refuses to apply an update whose release publishes no checksum). `T2_ZIP_ASSET_NAME`/`T3_ZIP_ASSET_NAME` constants are the only target-specific part. `write_success_flag(exe_dir)` (v1.7.2.4.1) — best-effort startup-handshake marker for `updater_helper.ps1`, called by both GUI entry points right after `window.show()`. Stdlib only, no exceptions to the Leaf-Node rule. |
...
### `tests/test_updater.py` — self-updater, T2 + T3 (v1.7.2.4, v1.7.2.4.1)
...
`prepare_update()`, T2 and T3 asset names both covered). **v1.7.2.4.1**
adds `updater.write_success_flag()` coverage, plus the first
