...
| `CONTEXT_RESYNC_PENDING_FILE` | `CONTEXT_DIR/_mcp_resync_pending.json` | Pending-resync marker for MCP SQLite proxy sync (v1.7.2.3) — written by `context_silo_repair._mark_pending_resync()` via `context_writer.write_file()`, consumed by `clients/mcp_update._apply_pending_context_resync()`, cleared by `context/context_collector._consume_context_resync_ack()` once acked. See `REFERENCE_CONTEXT.md`/`REFERENCE_MCP.md`. |
...
└── src/                        ← all source files (v1.6.0.1+)
    ...
    │   ├── context_silo_check.py   ← Leaf-Node. Read-only integrity scan — missing days, coordinate plausibility. check_context_archive() → dict (v1.7.2.3)
    │   └── context_silo_repair.py  ← Real re-fetch + re-write for flagged days, via context_api/context_writer. fix_coordinates() → dict (v1.7.2.3)
    │
